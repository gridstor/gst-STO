...
const CombinedWeatherChart = memo(function CombinedWeatherChart() {
  ...
  const [lineCustomization, setLineCustomization] = useState({
    temperatureForecast: { color: '#3B82F6', style: 'solid', visible: true },
    temperatureActual: { color: '#EF4444', style: 'solid', visible: true },
    temperatureNormal: { color: '#22C55E', style: 'solid', visible: true },
    hdd: { color: '#8B5CF6', style: 'solid', visible: true },
    cdd: { color: '#F59E0B', style: 'solid', visible: true },
  });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Adjust weather datetimes to next day at midnight to fix display offset
      // Weather data comes as 23:00 UTC (from HE->HB conversion), which displays as next day
      const adjustedData = combinedData.map(d => {
        const date = new Date(d.datetime);
        // If hour is 23 (11 PM), add 1 hour to get to midnight of next day
        if (date.getUTCHours() === 23) {
          date.setUTCHours(24, 0, 0, 0); // This rolls over to next day at midnight
        }
        return {
          ...d,
  ...
  const traces: any[] = useMemo(() => {
    ...
    // Temperature Forecast - Combined continuous line
    if (lineCustomization.temperatureForecast.visible) {
      traces.push({
        x: data.map(d => d.datetime),
        y: data.map(d => d.isLastWeek ? d.temperatureForecastLastWeek : d.temperatureForecastForecast),
        name: 'Temperature Forecast',
        type: 'scatter',
        mode: 'lines',
        yaxis: 'y',
        line: {
          color: lineCustomization.temperatureForecast.color,
          width: 2,
          dash: lineCustomization.temperatureForecast.style
        },
        hovertemplate: '<b>%{fullData.name}</b><br>%{y:.1f}°F<br><extra></extra>'
      });
    }

    // Temperature Actual (Last Week only)
    if (lineCustomization.temperatureActual.visible) {
      traces.push({
        x: lastWeekData.map(d => d.datetime),
        y: lastWeekData.map(d => d.temperatureActualLastWeek),
        name: 'Temperature Actual',
        type: 'scatter',
        mode: 'lines',
        yaxis: 'y',
        line: {
          color: lineCustomization.temperatureActual.color,
          width: 2,
          dash: lineCustomization.temperatureActual.style
        },
        hovertemplate: '<b>%{fullData.name}</b><br>%{y:.1f}°F<br><extra></extra>'
      });
    }

    // Temperature Normal (Last Week only)
    if (lineCustomization.temperatureNormal.visible) {
      traces.push({
        x: lastWeekData.map(d => d.datetime),
        y: lastWeekData.map(d => d.temperatureNormalLastWeek),
        name: 'Normal Temperature',
        type: 'scatter',
        mode: 'lines',
        yaxis: 'y',
        line: {
          color: lineCustomization.temperatureNormal.color,
          width: 2,
          dash: lineCustomization.temperatureNormal.style
        },
        hovertemplate: '<b>%{fullData.name}</b><br>%{y:.1f}°F<br><extra></extra>'
      });
    }
    ...
    // HDD (Last Week only)
    if (lineCustomization.hdd.visible) {
      traces.push({
        x: lastWeekData.map(d => d.datetime),
        y: lastWeekData.map(d => d.hddLastWeek),
        name: 'HDD',
        type: 'scatter',
        mode: 'lines',
        yaxis: 'y2',
        line: {
          color: lineCustomization.hdd.color,
          width: 2,
          dash: lineCustomization.hdd.style
        },
        hovertemplate: '<b>%{fullData.name}</b><br>%{y:.1f} degree-days<br><extra></extra>'
      });
    }

    // CDD (Last Week only)
    if (lineCustomization.cdd.visible) {
      traces.push({
        x: lastWeekData.map(d => d.datetime),
        y: lastWeekData.map(d => d.cddLastWeek),
        name: 'CDD',
        type: 'scatter',
        mode: 'lines',
        yaxis: 'y2',
        line: {
          color: lineCustomization.cdd.color,
          width: 2,
          dash: lineCustomization.cdd.style
        },
        hovertemplate: '<b>%{fullData.name}</b><br>%{y:.1f} degree-days<br><extra></extra>'
      });
    }
  ...
  // Plotly layout configuration with dual y-axes
  const layout: any = useMemo(() => {
    const layoutConfig: any = {
      height: 600,
      margin: { l: 80, r: 80, t: 40, b: 140 },
      title: {
        text: '',
        font: {
          size: 18,
  ...
  if (loading) {
    return (
      <div className="bg-white p-6 rounded-lg shadow-gs-sm border border-gs-gray-200 h-96 flex items-center justify-center">
        <div className="text-gs-gray-500">Loading weather data...</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`grid gap-4 ${showCustomization ? 'grid-cols-3' : 'grid-cols-1'}`}>
        ...
        <div className={`bg-white p-6 rounded-lg shadow-gs-sm border-l-4 border-gs-amber-500 ${showCustomization ? 'col-span-2' : ''}`} style={{ minHeight: '700px' }}>
          {/* Title and Customize Button */}
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xl font-semibold text-gs-gray-900">Weather Forecast</h3>
            <button
          ...
          {Plot && (
            <div style={{ width: '100%', height: '600px' }}>
              <Plot
                key={`weather-chart-${currentWeekScenarioId}-${lastWeekScenarioId}`}
                data={traces}
                layout={layout}
                config={config}
