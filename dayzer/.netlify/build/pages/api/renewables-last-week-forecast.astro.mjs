...
const GET = async ({ request }) => {
  ...
  try {
    ...
    const dayzerWindT1Map = {};
    const dayzerSolarT1Map = {};
    ...
    const renewablesData = Array.from(allDatetimes).sort().map((datetime) => {
      return {
        ...
        // Use 0 instead of null for missing data
        dayzerWindT1: dayzerWindT1Map[datetime] || 0,
        // t-1 Wind forecast
        dayzerSolarT1: dayzerSolarT1Map[datetime] || 0,
        // t-1 Solar forecast
        caisoWind: caisoWindMap[datetime] || 0,
        // Use 0 instead of null for missing data
        caisoSolar: caisoSolarMap[datetime] || 0,
        // Use 0 instead of null for missing data
        rtWind: rtWindMap[datetime] ? rtWindMap[datetime].sum / rtWindMap[datetime].count : 0,
        // RT Wind actuals averaged
        rtSolar: rtSolarMap[datetime] ? rtSolarMap[datetime].sum / rtSolarMap[datetime].count : 0
        // RT Solar actuals averaged
      };
    });
